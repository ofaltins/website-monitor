...
const runMonitor = async monitor => {
    ...
    console.log(`Running monitor ${monitor.description} at ${new Date().toString()}`)
    ...
    if (isValid) {
        console.log(`Monitor ${monitor.description} yields VALID with content ${content} at ${now}`)
        // Notify / send email here
        return true
    }

    console.log(`Monitor ${monitor.description} yields NOT VALID with content ${content} at ${now}`)
